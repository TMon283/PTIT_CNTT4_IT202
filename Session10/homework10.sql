...
-- Tạo một chỉ mục (index) trên cột username của bảng users.
create index idx_user_username on users(username);
-- Tạo một View có tên view_user_activity_2 để thống kê tổng số bài viết (total_posts) và tổng số bạn bè (total_friends) của mỗi người dùng. 
...
select u.user_id, count(p.content) as total_posts, count(fr.status) as total_friends from users u
join posts p on p.user_id = u.user_id
join friends fr on fr.user_id = u.user_id
where status = 'accepted'
group by u.user_id;
--  Hiển thị lại view trên.
select * from  view_user_activity_2;

-- Viết một truy vấn kết hợp view_user_activity với bảng users để hiển thị danh sách người dùng (bao gồm full_name, total_posts, total_friends), 
-- chỉ bao gồm người dùng có total_posts > 0 (số bài viết lớn hơn 0), sắp xếp theo total_posts giảm dần (từ cao đến thấp).
