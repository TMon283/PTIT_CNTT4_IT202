...
SELECT r.room_type, COUNT(r.room_type) AS 'Số lượng phòng' FROM rooms r
GROUP BY r.room_type;

-- PHẦN II – TRUY VẤN NÂNG CAO
-- Hãy liệt kê danh sách các lần đặt phòng, Với mỗi lần đặt phòng, hãy hiển thị: 1.Tên khách hàng 2.Loại phòng đã đặt 3.Ngày nhận phòng (check_in)
SELECT g.guest_name AS customer_name, r.room_type, b.check_in FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
JOIN rooms r ON b.room_id = r.room_id
ORDER BY b.check_in;

-- Cho biết mỗi khách đã đặt phòng bao nhiêu lần
SELECT b.guest_id, g.guest_name, COUNT(*) AS booking_count FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
GROUP BY b.guest_id, g.guest_name
ORDER BY booking_count DESC;

-- Tính doanh thu của mỗi phòng, với công thức: “Doanh thu = số ngày ở × giá thuê theo ngày”
SELECT r.room_id, r.room_type, SUM((b.check_out - b.check_in) * r.price_per_day) AS total_revenue FROM bookings b
JOIN rooms r ON b.room_id = r.room_id
GROUP BY r.room_id, r.room_type
ORDER BY total_revenue DESC;

-- Hiển thị tổng doanh thu của từng loại phòng
SELECT r.room_type, SUM((b.check_out - b.check_in) * r.price_per_day) AS revenue_by_type FROM bookings b
JOIN rooms r ON b.room_id = r.room_id
GROUP BY r.room_type
ORDER BY revenue_by_type DESC;

-- Thống kê số đơn hàng của mỗi khách hàng
SELECT g.guest_id, g.guest_name, COUNT(b.booking_id) AS booking_count FROM guests g
...
SELECT r.room_type, COUNT(*) AS cnt FROM bookings b
JOIN rooms r ON b.room_id = r.room_id
GROUP BY r.room_type
ORDER BY cnt DESC
LIMIT 1;
